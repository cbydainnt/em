import { Field } from '@nestjs/graphql';
import { InputType } from '@nestjs/graphql';
import { QuizCreateWithoutParent_quizInput } from './quiz-create-without-parent-quiz.input';
import { Type } from 'class-transformer';
import { QuizCreateOrConnectWithoutParent_quizInput } from './quiz-create-or-connect-without-parent-quiz.input';
import { QuizUpsertWithWhereUniqueWithoutParent_quizInput } from './quiz-upsert-with-where-unique-without-parent-quiz.input';
import { QuizCreateManyParent_quizInputEnvelope } from './quiz-create-many-parent-quiz-input-envelope.input';
import { Prisma } from '@prisma/client';
import { QuizWhereUniqueInput } from './quiz-where-unique.input';
import { QuizUpdateWithWhereUniqueWithoutParent_quizInput } from './quiz-update-with-where-unique-without-parent-quiz.input';
import { QuizUpdateManyWithWhereWithoutParent_quizInput } from './quiz-update-many-with-where-without-parent-quiz.input';
import { QuizScalarWhereInput } from './quiz-scalar-where.input';

@InputType()
export class QuizUncheckedUpdateManyWithoutParent_quizNestedInput {

    @Field(() => [QuizCreateWithoutParent_quizInput], {nullable:true})
    @Type(() => QuizCreateWithoutParent_quizInput)
    create?: Array<QuizCreateWithoutParent_quizInput>;

    @Field(() => [QuizCreateOrConnectWithoutParent_quizInput], {nullable:true})
    @Type(() => QuizCreateOrConnectWithoutParent_quizInput)
    connectOrCreate?: Array<QuizCreateOrConnectWithoutParent_quizInput>;

    @Field(() => [QuizUpsertWithWhereUniqueWithoutParent_quizInput], {nullable:true})
    @Type(() => QuizUpsertWithWhereUniqueWithoutParent_quizInput)
    upsert?: Array<QuizUpsertWithWhereUniqueWithoutParent_quizInput>;

    @Field(() => QuizCreateManyParent_quizInputEnvelope, {nullable:true})
    @Type(() => QuizCreateManyParent_quizInputEnvelope)
    createMany?: QuizCreateManyParent_quizInputEnvelope;

    @Field(() => [QuizWhereUniqueInput], {nullable:true})
    @Type(() => QuizWhereUniqueInput)
    set?: Array<Prisma.AtLeast<QuizWhereUniqueInput, 'quiz_id'>>;

    @Field(() => [QuizWhereUniqueInput], {nullable:true})
    @Type(() => QuizWhereUniqueInput)
    disconnect?: Array<Prisma.AtLeast<QuizWhereUniqueInput, 'quiz_id'>>;

    @Field(() => [QuizWhereUniqueInput], {nullable:true})
    @Type(() => QuizWhereUniqueInput)
    delete?: Array<Prisma.AtLeast<QuizWhereUniqueInput, 'quiz_id'>>;

    @Field(() => [QuizWhereUniqueInput], {nullable:true})
    @Type(() => QuizWhereUniqueInput)
    connect?: Array<Prisma.AtLeast<QuizWhereUniqueInput, 'quiz_id'>>;

    @Field(() => [QuizUpdateWithWhereUniqueWithoutParent_quizInput], {nullable:true})
    @Type(() => QuizUpdateWithWhereUniqueWithoutParent_quizInput)
    update?: Array<QuizUpdateWithWhereUniqueWithoutParent_quizInput>;

    @Field(() => [QuizUpdateManyWithWhereWithoutParent_quizInput], {nullable:true})
    @Type(() => QuizUpdateManyWithWhereWithoutParent_quizInput)
    updateMany?: Array<QuizUpdateManyWithWhereWithoutParent_quizInput>;

    @Field(() => [QuizScalarWhereInput], {nullable:true})
    @Type(() => QuizScalarWhereInput)
    deleteMany?: Array<QuizScalarWhereInput>;
}
