      ...
      if (data.success) {
        ...
        const userNotifications = data.data.filter((noti) => {
          ...
          if (noti.user_id === null) {
            return !noti.user_type || noti.user_type === 'user';
          }
  ...
  return (
    <div ref={menuRef}>
      ...
      >
        <IconButton
          onClick={handleClick}
          className="p-2 text-gray-700 dark:text-gray-200 hover:bg-violet-100 dark:hover:bg-gray-700 transition"
          sx={{
            position: 'relative',
          }}
        >
          <Badge badgeContent={displayUnreadCount} color="error" overlap="circular" anchorOrigin={{ vertical: 'top', horizontal: 'right' }}>
            <IconBell size={22} />
          </Badge>
        </IconButton>
      </Tooltip>
          ...
          <>
            ...
            <Box sx={{ p: 1, textAlign: 'center' }}>
              <Button
                ...
                onClick={() => {
                  window.location.href = '/tat-ca-thong-bao';
                  handleClose();
                }}
              >
                Tất cả thông báo
...
export default EM_NotificationMenu;
...
function NotificationItem({ notification, displayStatus, typeInfo, onMarkAsRead, onDelete, onClick, getTimeAgo, isSystemNotification }: NotificationItemProps) {
