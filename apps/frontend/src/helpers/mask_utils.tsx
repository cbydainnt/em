...
import { convertSegmentsToSVG, generatePolygonSegments } from "./custom_tracer";
/**
...
export const getAllMasks = (maskData: any, height: number, width: number) => {
  const masks = [];
  for (let m = 0; m < 4; m++) {
    const nthMask = new Float32Array(height * width);
...
export const rleFrString = (input: any) => {
  ...
  while (charIndex < input.length) {
    let value = 0,
      ...
      more = 1;
    while (more) {
      const c = input.charCodeAt(charIndex) - 48;
      value |= (c & 0x1f) << (5 * k);
      more = c & 0x20;
      charIndex++;
      k++;
...
export function keepArrayForMultiMask(
  ...
  keepArray = keepArray.map(
    (val: boolean, i: number) => val && duplicateArray[i]
  );
